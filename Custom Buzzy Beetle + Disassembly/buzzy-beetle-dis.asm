...
!miscProps = %00000000				; format: aw--jfls, default = %00000000 / $00
; these properties only seem to half work for the buzzy beetle, since most of these are intended for the koopas
; you can try to use them, but do so at your own risk
; a = animate faster in the air (seems to not be compatible with the buzzy beetle, i would leave it as 0)
; w = draw wings like a parakoopa (broken with buzzy beetles, it's intended for the parakoopas, leave as 0)
; - = unused (?)
; j = jump over shells
; f = follow/turn around to face mario
; l = stay on ledges
; s = use faster walk speeds
...
BuzzyBeetleMain:
	...
	LDA #!miscProps
	LSR
	LDY !157C,x						; \ load base walking speed based on horizontal direction
	BCC +							; |
	INY #2							; | use fast speeds if the misc property bit is set
	+								; |
	LDA walkXSpeed,y				; /
...
KickSprCarried:
	...
	LDA #$0A
	STA !sprite_status,x
	LDY $76							; \ get direction
	LDA $187A|!addr					; | and check if riding yoshi
	BEQ +							; | if not, use the slow throw speeds
	INY #2							; | otherwise, use fast spit speeds
	+								; | but since the beetle is immediately swallowed, this is unused
	LDA ShellSpeedX,y				; |
